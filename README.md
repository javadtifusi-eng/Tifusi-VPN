...
لینک همیشگی آخرین نسخه (هر بیلد جدید خودکار همین‌جا قرار می‌گیرد):

**https://github.com/javadtifusi-eng/Tifusi-VPN/releases/latest/download/tifusi-vpn.apk**

APK را روی گوشی باز کنید و اجازه‌ی «نصب از منابع ناشناس» را بدهید. همه‌ی نسخه‌ها با یک کلید ثابت امضا می‌شوند، پس روی نسخه‌ی قبلی نصب می‌شود و اطلاعات پاک نمی‌شود.

| فایل در Releases | برای چه گوشی‌ای | حجم |
|---|---|---|
| `tifusi-vpn.apk` | همه‌ی گوشی‌ها (universal) | بیشترین |
| `tifusi-vpn-arm64-v8a.apk` | تقریباً همه‌ی گوشی‌های امروزی (۶۴ بیتی) | حدود نصف |
| `tifusi-vpn-armeabi-v7a.apk` | گوشی‌های قدیمی ۳۲ بیتی | حدود نصف |

اگر مطمئن نیستید، همان `tifusi-vpn.apk` را بگیرید؛ روی همه‌ی گوشی‌ها نصب می‌شود. به‌روزرسانی داخل اپ (تب «درباره») خودش فایل متناسب با معماری همان گوشی را انتخاب می‌کند.

ساخت با Android Studio هم ممکن است: پوشه را باز کنید (Gradle 8.7، JDK 17) و `Build > Build APK(s)`. برای بیلد محلی باید `libv2ray.aar` نسخه‌ی پین‌شده در `.github/workflows/build-apk.yml` را در `app/libs/` بگذارید.
...
| **VLESS (Reality)** | ✅ لینک `vless://` | ✅ وصل/قطع خودکار با هسته‌ی Xray داخل اپ |
...
- **ارتباط با ما:** آیدی تلگرام `tifusi.supportTelegram` در `gradle.properties` (الان `@javadheydeari`) در تب پروفایل نمایش داده می‌شود.
...
│   ├── XrayVpnService.kt       # VLESS/Reality روی هسته‌ی Xray
...
## محدودیت‌های فعلی
- آمار ترافیک برای VLESS دقیق است؛ برای IKEv2 تخمینی است (اندروید شمارنده‌ی جدا برای تونل نمی‌دهد).
- WireGuard حذف شده است: روی بخشی از گوشی‌ها بد وصل می‌شد و کتابخانه‌اش هم حجم APK را بالا می‌برد. پروفایل‌های WireGuardِ ذخیره‌شده‌ی قدیمی موقع باز شدن اپ نادیده گرفته می‌شوند.
